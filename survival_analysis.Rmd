...
#selectCox(Surv(time, status)~ poc + age_during_show + personality_type_binary, data = survivor_data_final)
```

## Cox-proposional hazard model 1
```{r}
surv_model_cox1 <- coxph(data = survivor_data_final,
  Surv(time, status) ~  poc + age_during_show + gender + personality_type_binary + region)

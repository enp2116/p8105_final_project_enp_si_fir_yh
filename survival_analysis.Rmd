...
surv_model_full <- coxph(data = survivor_data_final,
                         Surv(time, status)~ poc + age_during_show + 
                           personality_type_binary + gender + region)
...
surv_model_cox1 <- coxph(data = survivor_data_final,
  Surv(time, status) ~  poc + age_during_show + personality_type_binary + gender + region)

summary(surv_model_cox1)

#Clean table for Cox PH
tabcoxph(surv_model_cox1, 
         var.labels = list(poc = "POC", age_during_show = "Age", personality_type_binary = "Personality Type", gender = "Gender", region = "Region"), 
         decimals = 2)
```
**HR interpretations:**
